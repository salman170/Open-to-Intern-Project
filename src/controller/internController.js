...
const createIntern = async (req, res) => {
    try {
        ...
        if (!validString.test(name)) { return res.status(400).send({ status: false, message : "name should be In A-Z or a-z " }) }


        let validEmail = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/
        if (!validEmail.test(email)) {
            return res.status(400).send({ status: false, message: "please enter email in  correct format  e.g  xyz@abc.com" })
        }
    
        let emailExist = await internsModel.findOne({ email: email })

        if (emailExist) { return res.status(400).send({ status: false, message : `This ${email} email is already exits pls sign In` }) }


        let validNumber = /^[6-9]{1}[0-9]{9}$/

        if (!validNumber.test(mobile)) { return res.status(400).send({ status: false, message : " please enter 10 digit IND mobile number 🇮🇳" }) }
        ...
        if (!collegeDetails) { return res.status(404).send({ status: false, message: `No college found with college name ${collegeName}` }) }
